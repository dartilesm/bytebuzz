revoke delete on table "public"."post_media"
from
  "anon";
...
revoke references on table "public"."post_media"
from
  "anon";
...
revoke trigger on table "public"."post_media"
from
  "anon";

revoke truncate on table "public"."post_media"
from
  "anon";
...
revoke delete on table "public"."reactions"
from
  "anon";
...
revoke references on table "public"."reactions"
from
  "anon";
...
revoke trigger on table "public"."reactions"
from
  "anon";

revoke truncate on table "public"."reactions"
from
  "anon";
...
revoke delete on table "public"."user_followers"
from
  "anon";

revoke
insert
  on table "public"."user_followers"
from
  "anon";

revoke references on table "public"."user_followers"
from
  "anon";
...
revoke trigger on table "public"."user_followers"
from
  "anon";

revoke truncate on table "public"."user_followers"
from
  "anon";
...
alter table
  "public"."posts"
add
  column "fts" tsvector generated always as (
    to_tsvector(
      'english' :: regconfig,
  ...
  column "fts" tsvector generated always as (
    to_tsvector(
      ...
      (
        (
          (
            (COALESCE(username, '' :: text) || ' ' :: text) || COALESCE(display_name, '' :: text)
          ) || ' ' :: text
        ) || COALESCE(github_url, '' :: text)
      )
    )
  ) stored;
...
CREATE INDEX users_fts_idx ON public.users USING gin (fts);

set
  check_function_bodies = off;

CREATE
OR REPLACE FUNCTION public.search_posts(
  search_term text,
...
IF limit_count <= 0 THEN limit_count := 20;

-- Default to 20 if invalid
END IF;

IF offset_count < 0 THEN offset_count := 0;
...
SELECT
  ...
  CASE
    WHEN p.repost_post_id IS NOT NULL THEN json_build_object(
      'id',
      rp.id,
      'content',
      rp.content,
...
WHERE
  (
    ...
    (
      ...
      AND LOWER(p.content) LIKE '%' || LOWER(search_term) || '%'
    )
    OR -- Fallback: Partial search in username
    LOWER(u.username) LIKE '%' || LOWER(search_term) || '%'
    OR -- Fallback: Partial search in display_name
    (
      u.display_name IS NOT NULL
      AND LOWER(u.display_name) LIKE '%' || LOWER(search_term) || '%'
    )
    OR -- Fallback: Partial search in github_url
    (
      u.github_url IS NOT NULL
      AND LOWER(u.github_url) LIKE '%' || LOWER(search_term) || '%'
    )
  )
  AND -- Only return top-level posts (not replies) for cleaner search results
...
ORDER BY
  ...
  (
    COALESCE(p.star_count, 0) + COALESCE(p.coffee_count, 0) + COALESCE(p.approve_count, 0) + COALESCE(p.cache_count, 0) + COALESCE(p.reply_count, 0) + COALESCE(p.repost_count, 0)
  ) DESC,
  -- Finally by creation date (newest first)
  p.created_at DESC
LIMIT
  limit_count OFFSET offset_count;
...
ORDER BY
  ...
  CASE
    WHEN LOWER(u.username) = LOWER(search_term) THEN 1
    ELSE 2
  END,
...
WHERE
  p.id IS NULL
  AND pm.created_at < NOW() - INTERVAL '24 hours' LOOP -- Log the file being processed
  RAISE NOTICE 'Processing orphaned file: %',
  orphaned_record.file_path;
...
DELETE FROM
  post_media
WHERE
  file_path = orphaned_record.file_path;
...
raise notice 'Successfully deleted file from bucket: %',
file_path;

exception
when others then -- Log any errors
raise notice 'Error deleting file %: %',
file_path,
sqlerrm;
...
OR REPLACE FUNCTION public.get_post_ancestry(start_id uuid) RETURNS TABLE(
  ...
  SELECT
    ...
    CASE
      WHEN parent.repost_post_id IS NOT NULL THEN json_build_object(
        'id',
        rp.id,
        'content',
        rp.content,
...
WHERE
  ...
  AND u.id NOT IN (
    ...
    FROM
      user_followers
    WHERE
      follower_id = authenticated_user
  )
ORDER BY
  random()
LIMIT
...
BEGIN authenticated_user := (auth.jwt() ->> 'sub');

-- Get user ID from JWT
RAISE NOTICE 'Authenticated User: %, Target User: %',
authenticated_user,
target_user_id;

IF authenticated_user IS NULL THEN RAISE EXCEPTION 'Current user is not authenticated';

END IF;

IF authenticated_user = target_user_id THEN RAISE EXCEPTION 'You cannot follow yourself';
...
SELECT
  EXISTS (
    ...
    WHERE
      ...
      AND follower_id = authenticated_user
  ) INTO is_following;

RAISE NOTICE 'Is Following: %, Authenticated User: %',
is_following,
authenticated_user;
...
VALUES
  (target_user_id, authenticated_user);
...
BEGIN -- Check if any reaction exists for that user + post
SELECT
  id,
  reaction_type INTO existing_id,
  existing_type
FROM
  reactions
WHERE
...
IF existing_id IS NOT NULL THEN IF existing_type = input_reaction_type THEN -- Same reaction: delete it (toggle off)
DELETE FROM
  reactions
WHERE
  id = existing_id RETURNING * INTO reaction_row;

RETURN reaction_row;

ELSE -- Different reaction: update to the new type
UPDATE
  reactions
SET
...
ELSE -- No reaction yet: insert new
INSERT INTO
  reactions (post_id, reaction_type)
VALUES
  (input_post_id, input_reaction_type) RETURNING * INTO reaction_row;
...
UPDATE
  users
SET
  follower_count = COALESCE(follower_count, 0) + 1
WHERE
  id = NEW.user_id;
...
ELSIF TG_OP = 'DELETE' THEN -- Decrease follower count of the unfollowed user
UPDATE
  users
SET
  follower_count = GREATEST(COALESCE(follower_count, 1) - 1, 0)
WHERE
  id = OLD.user_id;
...
UPDATE
  users
SET
  following_count = GREATEST(COALESCE(following_count, 1) - 1, 0)
WHERE
  id = OLD.follower_id;
...
IF TG_OP = 'INSERT' THEN -- Increment the appropriate counter
UPDATE
  posts
SET
  star_count = star_count + (
    CASE
      WHEN NEW.reaction_type = 'star' THEN 1
      ELSE 0
    END
  ),
  coffee_count = coffee_count + (
    CASE
      WHEN NEW.reaction_type = 'coffee' THEN 1
      ELSE 0
    END
  ),
  approve_count = approve_count + (
    CASE
      WHEN NEW.reaction_type = 'approve' THEN 1
      ELSE 0
    END
  ),
  cache_count = cache_count + (
    CASE
      WHEN NEW.reaction_type = 'cache' THEN 1
      ELSE 0
    END
  )
WHERE
...
ELSIF TG_OP = 'DELETE' THEN -- Decrement the appropriate counter
UPDATE
  posts
SET
  star_count = GREATEST(
    star_count - (
      CASE
        WHEN OLD.reaction_type = 'star' THEN 1
        ELSE 0
      END
    ),
    0
  ),
  coffee_count = GREATEST(
    coffee_count - (
      CASE
        WHEN OLD.reaction_type = 'coffee' THEN 1
        ELSE 0
      END
    ),
    0
  ),
  approve_count = GREATEST(
    approve_count - (
      CASE
        WHEN OLD.reaction_type = 'approve' THEN 1
        ELSE 0
      END
    ),
    0
  ),
  cache_count = GREATEST(
    cache_count - (
      CASE
        WHEN OLD.reaction_type = 'cache' THEN 1
        ELSE 0
      END
    ),
    0
...
ELSIF TG_OP = 'UPDATE' THEN -- Handle the case where the reaction type is updated
IF NEW.reaction_type IS DISTINCT
FROM
  OLD.reaction_type THEN -- Decrement the count for the old reaction type
UPDATE
  posts
SET
  star_count = GREATEST(
...
INSERT
  OR
UPDATE
  OF name ON storage.buckets FOR EACH ROW EXECUTE FUNCTION storage.enforce_bucket_name_length();

END IF;

-- Check if storage.delete_prefix_hierarchy_trigger function exists
IF EXISTS (
  SELECT
    1
  FROM
    pg_proc
  WHERE
    proname = 'delete_prefix_hierarchy_trigger'
    AND pronamespace = (
      SELECT
        oid
      FROM
...
-- Check if storage.objects_insert_prefix_trigger function exists
IF EXISTS (
  SELECT
    1
  FROM
    pg_proc
  WHERE
    proname = 'objects_insert_prefix_trigger'
    AND pronamespace = (
      SELECT
        oid
      FROM
...
-- Check if storage.objects_update_prefix_trigger function exists
IF EXISTS (
  SELECT
    1
  FROM
    pg_proc
  WHERE
    proname = 'objects_update_prefix_trigger'
    AND pronamespace = (
      SELECT
        oid
      FROM
...
-- Check if storage.update_updated_at_column function exists
IF EXISTS (
  SELECT
    1
  FROM
...
-- Check if storage.prefixes_insert_trigger function exists
IF EXISTS (
  SELECT
    1
  FROM
    pg_proc
  WHERE
    proname = 'prefixes_insert_trigger'
    AND pronamespace = (
      SELECT
        oid
      FROM
...
-- Check if storage.delete_prefix_hierarchy_trigger function exists (for prefixes)
IF EXISTS (
  SELECT
    1
  FROM
