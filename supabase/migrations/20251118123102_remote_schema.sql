...
create policy "Any user can see users"
on "public"."users"
as permissive
for select
to public
...
create policy "Users must delete their own profile"
on "public"."users"
as permissive
for delete
to authenticated
...
for insert
to authenticated
with check ((( SELECT (auth.jwt() ->> 'sub'::text)) = id));


create policy "Users must update their own profile"
on "public"."users"
as permissive
for update
to authenticated
