...
export function init(options: any, { caller }: { caller?: string } = {}) {
  if (!promise) {
...
async function _init(props: any) {
  ...
  if (!emojiVersion) emojiVersion = PickerProps.emojiVersion.value;
  if (!set) set = PickerProps.set.value;
  if (!locale) locale = PickerProps.locale.value;
  ...
  if (props.custom) {
    for (let i in props.custom) {
      ...
      const category = props.custom[index];
      const prevCategory = props.custom[index - 1];

      if (!category.emojis || !category.emojis.length) continue;

      if (!category.id) category.id = `custom_${index + 1}`;
      if (!category.name) category.name = I18n.categories.custom;
  ...
  let noCountryFlags: boolean | null = null;
  if (set === "native") {
  ...
  while (categoryIndex--) {
    ...
    if (category.id === "frequent") {
      ...
      if (!perLine) perLine = PickerProps.perLine.value;
